...
class AudioRecorder:
    def __init__(self, samplerate, channels, recording_channels):
        """
        Inicializa o gravador de áudio.
        
        Args:
            samplerate (int): A taxa de amostragem do áudio.
            channels (int): O número de canais de áudio.
            recording_channels (list): Lista de índices dos canais a serem gravados.
        """
        self.samplerate = samplerate
        self.channels = channels
        self.recording_channels = recording_channels
        
        # O número de canais do arquivo de saída será igual ao número de canais selecionados
        self.output_channels = len(recording_channels)
    ...
    def start(self, filepath):
        ...
        if not self.recording_channels:
            print("Erro: Nenhum canal de áudio selecionado para gravação.")
            return False
    ...
    def write(self, data: np.ndarray):
        if self.is_active and not self.is_paused and not self.q.full():
            # Seleciona apenas os canais a serem gravados
            if self.output_channels > 0:
                recorded_data = data[:, self.recording_channels]
                self.q.put(recorded_data.copy())
