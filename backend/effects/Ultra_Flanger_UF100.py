...
class BehringerUf100Flanger:
    """
    Simulação do pedal Ibanez FL9 Flanger — conhecido por seu sweep suave,
    faixa de delay mais larga e resposta musical.
    """
    ...
    def __init__(self, sample_rate: int):
        ...
        # Faixa de delay típica do FL9
        self.min_delay_ms = 1.0
        self.max_delay_ms = 12.8

        max_delay_samples = int(self.max_delay_ms / 1000 * self.sample_rate)
    ...
    def apply(self, input_buffer: np.ndarray, **params) -> np.ndarray:
        ...
        for i, sample in enumerate(input_buffer):
            # LFO com curva suavizada para sweep orgânico
            lfo_value = np.sin(self.lfo_phase) ** 3
            ...
            feedback_gain = self.resonance * 0.9
            ...
            # Mix levemente mais suave
            output_buffer[i] = 0.6 * sample + 0.4 * delayed_sample
